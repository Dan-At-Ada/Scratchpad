...
export default function RSADemo() {
  ...
  const runDemo = async () => {
    ...
    appendLog(`Public Key (e, n): e=${publicKey.e}, n=${publicKey.n}`)
    appendLog(`Private Key (d, n): d=${privateKey.d}, n=${privateKey.n}`)
    appendLog(`Prime p: ${p}`)
    appendLog(`Prime q: ${q}`)
    ...
    if (factorP && factorQ) {
      ...
      appendLog(`Reconstructed Private Key: d=${crackedPrivateKey.d}, n=${crackedPrivateKey.n}`)

      appendLog("\nStep 6: Decrypting the message using the cracked key")
      const crackedDecrypted = await rsa.decrypt(encrypted, crackedPrivateKey)
      appendLog(`Decrypted Message using cracked key: ${crackedDecrypted}`)
